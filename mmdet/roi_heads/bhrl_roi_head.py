...
class BHRLRoIHead(BaseRoIHead, BBoxTestMixin, MaskTestMixin):
    ...
    def _bbox_forward(self, stage, x, img_feat, ref_roi_feats, rois,
        ...
        target_feats = bbox_roi_extractor(img_feat[:bbox_roi_extractor.num_inputs],  # 去掉了16尺寸
                                          rois)  # rois: (2048, 5)

        cls_score, bbox_pred = bbox_head(target_feats, rois=rois, query=ref_roi_feats)  # IHR在这  yuqi
    ...
    def _bbox_forward_train(self, stage, x, img_feat, ref_roi_feats, sampling_results, gt_bboxes,
                            gt_labels, rcnn_train_cfg,):  # yuqi
        """Run forward function and calculate loss for box head in training."""
        rois = bbox2roi([res.bboxes for res in sampling_results])
        bbox_results = self._bbox_forward(stage, x, img_feat, ref_roi_feats, rois)  # ihr在这里边

        bbox_targets = self.bbox_head[stage].get_targets(  # 根据采样结果计算一批样本的真实标签和回归目标
    ...
    def forward_train(self,
        ...
        losses = dict()
        for i in range(self.num_stages):  # self.num_stages=1
    ...
    def simple_test(self, x, img_feat, ref_roi_feats, proposal_list, img_metas, rescale=False):
        ...
        for i in range(self.num_stages):
            ...
            cls_score = bbox_results['cls_score']  # (num_proposal, 2)
